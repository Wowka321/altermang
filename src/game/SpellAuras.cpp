...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleNoImmediateEffect,                         //259 SPELL_AURA_DECREASE_PERIODIC_HEAL               implemented in Unit::SpellHealingBonusTaken
        ...
        if (Real && !target->HasAuraType(SPELL_AURA_MOD_STEALTH))
        {
            ...
            for(Unit::AuraList::const_iterator i = mDummyAuras.begin();i != mDummyAuras.end(); ++i)
            {
                ...
                // Overkill
                    ...
                    target->CastSpell(target, 58428, true); // Overkill we should remove in any case
            }
        }
    }
}
...
//                pCaster->ProcDamageAndSpell(target, procAttacker, procVictim, PROC_EX_NORMAL_HIT, pdamage, BASE_ATTACK, spellProto);
        ...
        case SPELL_AURA_PERIODIC_MANA_LEECH:
        {
            ...
            if(GetSpellProto()->Id == 31447)
                if(pdamage>target->GetPower(POWER_MANA))
                    explode = true;
            ...
            if(explode)
                pCaster->CastSpell(target,31463,true,0,this);
