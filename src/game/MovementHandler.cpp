...
void WorldSession::HandleMovementOpcodes( WorldPacket & recv_data )
{
    ...
    if(plMover)
    {
        sLog.outBasic("MA-%s > client-time:%d fall-time:%d | xyzo: %f,%f,%fo(%f) flags[%X] opcode[%s]| transport (xyzo): %f,%f,%fo(%f)",
                    plMover->GetName(),movementInfo.time,movementInfo.fallTime,movementInfo.GetPos()->x,movementInfo.GetPos()->y,movementInfo.GetPos()->z,plMover->GetOrientation(),
                    movementInfo.GetMovementFlags(), LookupOpcodeName(opcode),movementInfo.GetTransportPos()->x,movementInfo.GetTransportPos()->y,movementInfo.GetTransportPos()->z,movementInfo.GetTransportPos()->o);
    ...
    else
    {
        sLog.outBasic("MA > client-time:%d fall-time:%d | xyzo: %f,%f,%fo(%f) flags[%X] opcode[%s]| transport (xyzo): %f,%f,%fo(%f)",
                    movementInfo.time,movementInfo.fallTime,movementInfo.GetPos()->x,movementInfo.GetPos()->y,movementInfo.GetPos()->z,plMover->GetOrientation(),
                    movementInfo.GetMovementFlags(), LookupOpcodeName(opcode),movementInfo.GetTransportPos()->x,movementInfo.GetTransportPos()->y,movementInfo.GetTransportPos()->z,movementInfo.GetTransportPos()->o);
        sLog.outBasic("MA Transport > server GUID:  |  client GUID: (lo)%d - (hi)%d",
                    GUID_LOPART(movementInfo.t_guid.GetRawValue()), GUID_HIPART(movementInfo.t_guid.GetRawValue()));
    }
    #endif
    ...
    if (plMover && World::GetEnableMvAnticheat())
    {
        ...
        if ((plMover->m_anti_TransportGUID == 0) && !curDest)
        {
            ...
            {
                #ifdef MOVEMENT_ANTICHEAT_DEBUG
                    sLog.outError("MA-%s, flight exception. {SPELL_AURA_FLY=[%X]} {SPELL_AURA_MOD_FLIGHT_SPEED_MOUNTED=[%X]} {SPELL_AURA_MOD_FLIGHT_SPEED_STACKING=[%X]} {SPELL_AURA_MOD_FLIGHT_SPEED_MOUNTED_STACKING=[%X]} {SPELL_AURA_MOD_FLIGHT_SPEED_NOT_STACKING=[%X]}",
                       plMover->GetName(),
                       plMover->HasAuraType(SPELL_AURA_FLY), plMover->HasAuraType(SPELL_AURA_MOD_FLIGHT_SPEED_MOUNTED),
                       ...
                       plMover->HasAuraType(SPELL_AURA_MOD_FLIGHT_SPEED_NOT_STACKING));
                #endif
                check_passed = false;
            }
                  ...
                  && !(plMover->HasAuraType(SPELL_AURA_WATER_WALK) | plMover->HasAuraType(SPELL_AURA_GHOST)))
            {
                    ...
                    sLog.outError("MA-%s, water-walk exception. [%X]{SPELL_AURA_WATER_WALK=[%X]}",
                                    plMover->GetName(), movementInfo.GetMovementFlags(), plMover->HasAuraType(SPELL_AURA_WATER_WALK));
                #endif
                check_passed = false;
            }
    ...
    if (check_passed)
    {
        ...
        if(plMover)                                             // nothing is charmed, or player charmed
        {
            ...
            {
                sLog.outBasic("MA-%s produce %d anticheat alarms",plMover->GetName(),plMover->m_anti_AlarmCount);
...
void WorldSession::HandleMoveKnockBackAck( WorldPacket & recv_data )
{
    ...
    #ifdef MOVEMENT_ANTICHEAT_DEBUG
        sLog.outBasic("%s CMSG_MOVE_KNOCK_BACK_ACK: tm:%d ftm:%d | %f,%f,%fo(%f) [%X]",GetPlayer()->GetName(),movementInfo.time,movementInfo.fallTime,movementInfo.GetPos()->x,movementInfo.GetPos()->y,movementInfo.GetPos()->z,_player->GetOrientation(),movementInfo.GetMovementFlags());
        sLog.outBasic("%s CMSG_MOVE_KNOCK_BACK_ACK additional: vspeed:%f, hspeed:%f",GetPlayer()->GetName(), movementInfo.j_velocity, movementInfo.j_xyspeed);
    #endif
