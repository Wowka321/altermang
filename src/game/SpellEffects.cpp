    ...
    if( unitTarget && unitTarget->isAlive())
    {
        switch(m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DRUID:
            {
                ...
                {
                    Aura *ImprovedAura =  m_caster->HasAura(57849) ? m_caster->GetAura(57849,EFFECT_INDEX_0) : m_caster->HasAura(57850) ?
                    m_caster->GetAura(57850,EFFECT_INDEX_0) : m_caster->HasAura(57851) ? m_caster->GetAura(57851,EFFECT_INDEX_0) : NULL;
...
void Spell::EffectHeal(SpellEffectIndex /*eff_idx*/)
{
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        else if(m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN && m_spellInfo->SpellFamilyFlags == UI64LIT(0x100))
        {
            ...
            {
                addhealth += addhealth * riptide->GetSpellProto()->EffectBasePoints[EFFECT_INDEX_2] / 100;
...
void Spell::EffectLeapForward(SpellEffectIndex eff_idx)
{
    ...
    if( m_spellInfo->rangeIndex == 1)                       //self range
    {
        ...
        if (unitTarget->GetTypeId() == TYPEID_PLAYER && ((Player*)unitTarget)->HasMovementFlag(MovementFlags(MOVEFLAG_FALLINGFAR |
            MOVEFLAG_SWIMMING | MOVEFLAG_FLYING | MOVEFLAG_WATERWALKING)))
            isFallorFly = true;
        ...
        if(isFallorFly)
        {
            ...
            if(((Player*)unitTarget)->HasMovementFlag(MOVEFLAG_FALLINGFAR))
            {
