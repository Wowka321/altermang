...
void Player::SetSkill(uint16 id, uint16 currVal, uint16 maxVal, uint16 step /*=0*/)
{
    ...
    if(itr != mSkillStatus.end() && itr->second.uState != SKILL_DELETED)
    {
        if(currVal)
        {
            _RemoveAllItemMods();                          // Maybe now we meet requirements, so reaplly mods
        ...
        {
            _RemoveAllItemMods();                          // And now not we can !meet, so reaplly mods again
            // clear skill fields
...
bool Player::SetPosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    if( teleport || old_x != x || old_y != y || old_z != z || old_r != orientation )
    {
        ...
        if(teleport || !(m_movementInfo.GetMovementFlags() & (MOVEFLAG_FALLING | MOVEFLAG_FALLINGFAR)))
        {
            m_safeposition.x = x;
            m_safeposition.y = y;
            m_safeposition.z = z;
